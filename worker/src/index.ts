...
export interface Env {
  RESEND_API_KEY_AHMED_RASHEDY: string;
  FROM_EMAIL: string;
  TO_EMAIL: string;
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (!env.RESEND_API_KEY_AHMED_RASHEDY) {
      return jsonResponse(
        {
          success: false,
          error:
            "Email service not configured. Set RESEND_API_KEY_AHMED_RASHEDY.",
        },
        500,
        corsHeaders,
      );
    ...
    try {
      ...
      const resend = new Resend(env.RESEND_API_KEY_AHMED_RASHEDY);
